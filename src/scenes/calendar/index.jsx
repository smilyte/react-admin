    ...
    const handleDateClick = (selected) => {
        ...
        if(title){
            calendarApi.addEvent({
                id: `${selected.dateStr}-${title}`, title, start: selected.startStr,
                end: selected.endStr, allDay:selected.allDay
            });
        }
    };
                ...
                <Box flex="1 1 20%" backgroundColor={colors.primary[400]} p="15px" borderRadius="4px">
                    ...
                    <List>
                        {currentEvents.map((event) => (
                            <ListItem
                                key={event.id} sx={{backgroundColor: colors.greenAccent[500], 
